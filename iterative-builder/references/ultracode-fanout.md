...
Fan-out **augments** the workflow — it never weakens it. Every user-approval gate stays (1.5 and 3.2 hard; 2.4 the conditional card gate — auto-proceed on routine cards, pause on sensitive/complex), `feature-dev:code-reviewer` stays the single authoritative review gate, the Codex second opinion stays advisory (now always run, no prompt), and the context reset after each task stays (a full `/clear` + re-invoke after every task). Read the Invariants section before changing any recipe.
...
- **The context reset stays (Rule 14).** After every task the skill stops for a full `/clear` + re-invoke, so the next task starts from a clean, disk-backed context. A background fan-out is confined to the current step of the current task and must be aggregated before the step that follows; it never runs across the reset.
...
**Discovery cards.** A `Change Type: discovery` card carries findings, not source. Critics (a)–(c) (scope-language, stub-risk/substance, Must-Haves shape) judge it on whether its findings are concrete, complete, and name the requirements unblocked — they do NOT FAIL it for findings-style Truths, an absent wiring Key Link, or a missing anti-stub constraint on `DISCOVERY-NN.md`. This mirrors the Step 7 reviewer's discovery carve-out (SKILL.md → Discovery Cards). Critic (d) confirms the findings output is concrete (rather than a behavioral code check), while critics (e) locked-decision/leakage and (f) sizing apply unchanged.
...
**Aggregation rule (conservative, mirrors the reviewer gate).** Aggregate PASS only if EVERY critic returns PASS. ANY FAIL = aggregate FAIL. On FAIL, take the UNION of all critics' issues (do not dismiss, downgrade, or de-duplicate away any finding) and re-invoke `feature-dev:code-architect` (loop back to Step 2.2) with that consolidated, specific feedback — preserving the existing "re-invoke code-architect with specific feedback" behavior. Then re-run the critic fan-out on the revised card. Cap this loop at 3 rounds; if the card still aggregates FAIL after the 3rd architect revision, stop and escalate per Edge Cases → "Card fails quality-check after 3 rounds" (present the persistent union of issues). There is no separate verifier that filters findings.
