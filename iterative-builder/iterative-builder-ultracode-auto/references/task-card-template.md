...
2. **Deviation classification.** Auto-fix: bugs, missing critical functionality, blocking deps. Stop and ask: architectural changes, new tables, breaking contracts. After 3 failed auto-fix attempts, stop and report. ("Stop and ask" means report back to the orchestrator — not a human. The orchestrator then applies the autonomy policy: auto-fix where safe, or HALT for a human only when the change is genuinely dangerous/risky — see SKILL.md → Autonomous Operation.)
